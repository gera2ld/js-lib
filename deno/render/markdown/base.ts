...
import { noop, once } from "https://cdn.jsdelivr.net/npm/es-toolkit@1.32.0/+esm";
...
export class MarkdownRenderer {
  ...
  static async create(plugins = builtInPlugins) {
    ...
    return new MarkdownRenderer(plugins, initMarkdownIt(highlighters), highlighters);
  }
  constructor(public plugins: IRenderPlugin[], private md: MarkdownIt, readonly highlighters: Record<string, (code: string) => string>) {
    plugins.forEach(({
      name,
      markdown
    }) => {
